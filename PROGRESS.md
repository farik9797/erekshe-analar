...
## Стандарт полустационара — казахский перевод (готово, задеплоено)
- Данные `src/data/standards.ts` и `wordpress/erekshe-theme/inc/standards-data.php` переведены в двуязычный вид `{ru,kk}` (`StdBi`): 6 разделов, ~17 пунктов, приложения 1-5 и все 136 услуг + нормы белья/длительность занятий/категории получателей.
- `StandardsPage.tsx` рендерит `section.title[lang] / c.text[lang] / b[lang] / STANDARDS_SOURCE[lang]`.
...
## /documents — убран дубль заголовка (готово, задеплоено)
Проблема: заголовок «Официальные документы и отчёты» выводился дважды — баннером страницы и шапкой `DocumentsSection`.
- React: `DocumentsSection` получил проп `hideHeader?: boolean`; `DocumentsPage.tsx` передаёт `<DocumentsSection hideHeader />`.
  Когда шапка скрыта, верхний отступ секции уменьшен (`pt-8 md:pt-10` вместо `py-16 md:py-24`).
- WP: `page-documents.php` → `get_template_part(..., ['hide_header' => true])`; в `section-documents.php` читается `$args['hide_header']`.
  Отступ задан **инлайн-стилем** (`padding-top:2rem`), т.к. классов `pt-10/pb-16/pb-24` нет в прособранном `assets/css/tailwind.css` темы.
- На `/about` и `/charity` шапка секции остаётся (там баннеры с другими заголовками) — проверено, регрессии нет.
- Проверено на проде (мобайл 375, ҚАЗ): заголовок один, карточка одна, overflow 0, русский текст не протекает.
- Коммиты: main `6a11757`, gh-pages `558c4b8`, бандл `index-DV-oJ-Fc.js`.

## /about — показатель специалистов 45+ → 32 (готово, задеплоено)
- `src/pages/AboutPage.tsx`: карточка показателей «45+» → «32» (подпись «Сертифицированных врачей и педагогов» / «Сертификатталған дәрігерлер мен педагогтар» не менялась).
  Теперь согласовано с hero главной (`heroStatTeam` = 32).
- В React других «45+» нет; на странице «Команда» счётчик динамический — `TEAM_ROSTER.length` (73).
- **Открытый вопрос:** в WP-теме остались хардкод-бейджи «45+ специалистов» / «45+ білікті маман» на баннере команды —
  `inc/i18n-extra-p.php` (p_teamBannerBadge), `inc/i18n-extra-c.php` (c_TeamBadgeSpecialists),
  `inc/i18n-extra-a.php` (a_teamBadge45), дефолт в `inc/acf-fields.php`. Не трогал: непонятно, 32 там или 73 (по ростеру).
- Коммиты: main `cabe871`, gh-pages `12fa139`, бандл `index-BTqjjw2L.js`.
